...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:umi="http://www.umi-cms.ru/TR/umi">
	...
	<xsl:template match="udata" mode="popular_offers_s1">
		<xsl:choose>
			<xsl:when test="$сatalog = 1">
				<xsl:apply-templates select="page" mode="simple_shop_catalog_popular_offers_s1"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<!--Элемент списка (ДЛЯ popular_offers_s1) -->
	<xsl:template match="item|page" mode="simple_shop_catalog_popular_offers_s1">
